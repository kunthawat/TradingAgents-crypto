...
def run_analysis_background(session_id: str, config: Dict):
    ...
    try:
        ...
        updated_config.update({
            ...
            'quick_think_llm': config.get('shallow_thinker', DEFAULT_CONFIG['quick_think_llm']),  # Map frontend name to backend config
            'deep_think_llm': config.get('deep_thinker', DEFAULT_CONFIG['deep_think_llm']),      # Map frontend name to backend config
